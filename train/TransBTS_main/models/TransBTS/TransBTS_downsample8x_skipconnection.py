...
import sys
sys.path.append("/data/sgc/cell_seg_49/pytorch_fnet-release_1/net/nn_modules/TransBTS_main")
...
class BTS(TransformerBTS):
    def __init__(
        ...
        self.endconv =nn.Conv3d(self.embedding_dim // 32, 1, kernel_size=3, padding=1)
...
def Net(dataset='brats', _conv_repr=True, _pe_type="learned"):
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        x = torch.rand((4, 1, 32, 128, 128), device=cuda0)
        model = Net(dataset='brats', _conv_repr=True, _pe_type="learned")
        model.cuda()
        y = model(x)
        print(y.shape)
