...
from dataset2D import MicroDLDataset
...
def run_ddp(rank, world_size, dist_url, dist_backend, device_str):
    ...
    for epoch in range(MDL_trainer.current_epoch, args.total_epoch):    # actually , the epoch start from 1 since every load state from .yingmuzhi file ,epoch will +1.
        ...
        if rank == 0:
            ...
            # save checkpoint
            MDL_trainer.save_state(args.checkpoint_path, save_best_path=args.best_checkpoint_path)
...
def run_single():
    ...
    if os.path.exists(args.checkpoint_path):
        MDLtrainer = trainer.microDL_trainer_2d.MicroDLTrainer.load_state_from_yingmuzhi(
            path_model_dir=args.checkpoint_path,
            gpu_ids=args.gpu_ids,
        )
        print("LOAD::PRE-TRAINED MODEL SUCCESSFULLY.")
    else:
        MDLtrainer = trainer.microDL_trainer_2d.MicroDLTrainer(
            nn_module=args.nn_module,
            nn_kwargs=args.nn_kwargs,
            gpu_ids=args.gpu_ids,
            scaler_bool=args.scaler_bool,
    ...
    MDLtrainer.get_parameter_number(MDLtrainer.net, param_type_str="float32")
    MDLtrainer.get_flops(MDLtrainer.net, input_tensor=torch.randn((1, 1, 256, 256)))  # also depend on your input_tensor


    # 5) iteration
    for epoch in range(MDLtrainer.current_epoch, args.total_epoch):
        # training
        train_mean_loss, lr ,training_time= MDLtrainer.train_one_epoch(train_loader)

        # validation
        validation_mean_loss, metrics = MDLtrainer.evaluate_one_epoch(validation_loader, TARGET_MEDIAN= TARGET_MEDIAN, TARGET_IQR= TARGET_IQR)

        # store history
        df_path = args.csv_path
        MDLtrainer.save_history(df_path, train_mean_loss, lr, validation_mean_loss, metrics, training_time)

        # save checkpoint
        MDLtrainer.save_state(args.checkpoint_path, validation_mean_loss, save_best_path=args.best_checkpoint_path)
...
if __name__ == "__main__":
    json_file_path = "/home/yingmuzhi/microDL_3_0/src/config/ddp/2DUnet.json"
