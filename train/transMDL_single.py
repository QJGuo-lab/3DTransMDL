...
import pdb
import sys
sys.path.append("/home/yingmuzhi/microDL_3_0/src/net")
from TransBTS_main.models.TransBTS.Transformer import TransformerModel
from TransBTS_main.models.TransBTS.PositionalEncoding import FixedPositionalEncoding,LearnedPositionalEncoding
from TransBTS_main.models.TransBTS.Unet_skipconnection import Unet
...
class _Net_recurse(torch.nn.Module):
    def __init__(self, n_in_channels, mult_chan=2, depth=0):
        ...
        if depth > 1:
            ...
            self.conv_x = torch.nn.Conv3d(256, 512, kernel_size=3,stride=1,padding=1)
            self.position_encoding = LearnedPositionalEncoding(
            #    1,512,1, #self.seq_length, self.embedding_dim, self.seq_length
               dim_of_token=512, 
               num_of_token=512,
            )
            ...
            self.transformer = TransformerModel(
                512,#embedding_dim,
                # 6, # num_layers
                2,# num_layers, for sufficiently
                8,# num_heads,
                2048,# hidden_dim,
            0.1, 0.1,)
          
            self.pre_head_ln = nn.LayerNorm(512)
        self.convt2 = torch.nn.ConvTranspose3d(128, 128, kernel_size=2, stride=2)
        self.convt3 = torch.nn.ConvTranspose3d(512, 256, kernel_size=2, stride=2)
            
    def forward(self, x):
        if self.depth == 1:
            # return self.sub_2conv_more(x)

            # --- delete: TransBTS::before 
            # x = self.conv_x(x)  #[2, 128, 4, 32, 32] -> [2, 256, 4, 32, 32]
            # x = x.permute(0, 2, 3, 4, 1).contiguous()  # 256 4 32 32 [2, 256, 4, 32, 32] -> [2, 4, 32, 32, 256]
            # x = x.view(x.size(0), -1, 512)  # [2, 4, 32, 32, 256] -> [2, 4096, 256]
            # ---

            x = self.conv_x(x)                          # torch.Size([2, 256, 2, 16, 16]) -> torch.Size([2, 512, 2, 16, 16])
            x = x.permute(0, 2, 3, 4, 1).contiguous()   # torch.Size([2, 512, 2, 16, 16]) -> torch.Size([2, 2, 16, 16, 512])
            x = x.view(x.size(0), -1, 512)              # torch.Size([2, 2, 16, 16, 512]) -> torch.Size([2, 512, 512])
            ...
            x=x.view( x.size(0),
            2,#int(32 / self.patch_dim),
            16,#int(self.img_dim / self.patch_dim),
            16,#int(self.img_dim / self.patch_dim),
            512,#elf.embedding_dim,
            )   # [2, 4096, 256] -> [2, 4, 32, 32, 256]
            x = x.permute(0, 4, 1, 2, 3).contiguous()   # [2, 4, 32, 32, 256] -> [2, 256, 4, 32, 32]
            return x
        ...
        elif  self.depth==3 :  # depth > 0
           # x_2conv_more = self.sub_2conv_more(x)
            x_down=self.sub_down(x)
            x_sub_u= self.sub_u(x_down)
            
            x_convt = self.convt2(x_sub_u)
            x_cat = torch.cat((x_convt, x), 1)  # concatenate  384

            x_2conv_less = self.sub_2conv_less(x_cat)
        
        elif  self.depth==2 :  # depth > 0
           # x_2conv_more = self.sub_2conv_more(x)
            x_down=self.sub_down(x)
            x_sub_u= self.sub_u(x_down)
            
            x_convt = self.convt3(x_sub_u)
            x_cat = torch.cat((x_convt, x), 1)  # concatenate  384
...
def main():
    ...
    params_dict = get_parameter_number(net)
    ...
    pass    # the total number of params is 10580467, trainable params is 10580467.
            # the size of params is 40.36127853393555 MB
